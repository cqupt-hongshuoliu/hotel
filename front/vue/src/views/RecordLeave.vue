      ...
      <el-table-column fixed="right" label="操作" width="120">
        <template slot-scope="scope">
          ...
          <el-popconfirm
              ...
              title="您确定删除吗"
              @confirm = "handleDel(scope.row.leaveId)"
          >
...
export default {
  ...
  methods: {
    load(){
      this.request.get("/record-leave/page",{
        params: {
    ...
    save(){
      this.request.post("/record-leave/save", this.form).then(res => {
        if (res.code === '200'){
    ...
    handleDel(id){
      this.request.delete("/record-leave/" + id).then(res => {
        if (res.code === '200'){
